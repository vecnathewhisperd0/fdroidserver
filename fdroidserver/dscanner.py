...
class DockerDriver(object):
    ...
    class Commands:
        ...
        run = ['docker', 'run', '-e', '"EMULATOR={0}"'.format(DockerConfig.EMULATOR), '-e',
    ...
    def _adb_install_apk(self, apk_path):
        ...
        exec_id = self.cli.exec_create(self.container_id, 'adb install {0}'.format(apk_path))['Id']
    ...
    def _adb_uninstall_apk(self, app_id):
        ...
        via its app_id.
        """
        logging.info("Uninstalling {app_id} from the emulator.".format(app_id=app_id))
        exec_id = self.cli.exec_create(self.container_id, 'adb uninstall {0}'.format(app_id))['Id']
        output = self.cli.exec_start(exec_id).decode('utf-8')
    ...
    def _verify_apk_install(self, app_id):
        """
        Checks that the app_id is installed on the device running in the container.
        """
        logging.info("Verifying {app} is installed on the device.".format(app=app_id))
        exec_id = self.cli.exec_create(self.container_id, self.Commands.pm_list)['Id']
        output = self.cli.exec_start(exec_id).decode('utf-8')

        if ("Could not access the Package Manager" in output or
                "device offline" in output):
            logging.info("Device or package manager isn't up, so apk can't exist.")
    ...
    def _delete_file(self, path):
        """
        Deletes a file off the container to preserve space if scanning many apps.
        """
    ...
    def _install_drozer(self):
        ...
        """
        logging.info("Attempting to install drozer (com.mwr.dz) on the emulator")
    ...
    def _run_drozer_scan(self, app):
        ...
        exec_id = self.cli.exec_create(self.container_id,
                                       self.Commands.run_drozer.format(app))['Id']
        self.cli.exec_start(exec_id)
    ...
    def _build_docker_image(self):
        ...
        sleep(1) # we pause briefly to narrow race condition windows of opportunity

        is_a_tty = os.isatty(sys.stdout.fileno())

        for output in self.cli.pull(DockerConfig.CONTAINER,stream=True,tag="latest"):
            if not is_a_tty:
            ...
            try:
                ...
                for k in sorted(self._image_queue):
                    ...
                    t += vd['total']
                    c += vd['current']
                msg = "\rDownloading: {0}/{1} {2}% [{3} jobs]".format(c,t,int(c/t*100),j)
    ...
    def init_docker(self):
        ...
        if not running:
            ...
            # try to run the container, which will create if missing and die
            # if exists already
            try:
                check_output(self.Commands.run)
            except CalledProcessError as e:
                logging.error(('Command "{command}" failed with error code {code}'.format(command=self.Commands.run, code=e.returncode)))
            running = self._container_is_running()

        if not running:
            logging.info('Trying to start container...')
            # container most likely exists, just not started so let's try that now
            try:
...
def main():
    ...
    parser.add_argument("app_id", nargs='*', help="app-id with optional versioncode in the form APPID[:VERCODE]")
    parser.add_argument("-l", "--latest", action="store_true", default=False,
                        help="Scan only the latest version of each package")
    parser.add_argument("--clean-after", default=False, action='store_true',
                        help="Clean after all scans have finished")
    parser.add_argument("--clean-before", default=False, action='store_true',
                        help="Clean before the scans start and rebuild the container")
    parser.add_argument("--clean-only", default=False, action='store_true',
    ...
    parser.add_argument("--repo-path", default="repo", action="store",
    ...
    for app_id,app in apps.items():
        for build in app.builds:
            ...
            for f in os.listdir(options.repo_path):
                if f == str(app_id)+'_'+build.vercode+'.apk':
                    apks.append(f)
