...
from fdroidserver import _, net, vcs
...
def getvcs(vcstype, remote, local):
    if vcstype == 'git':
        return vcs.vcs_git(remote, local)
    if vcstype == 'git-svn':
        return vcs.vcs_gitsvn(remote, local)
    if vcstype == 'hg':
        return vcs.vcs_hg(remote, local)
    if vcstype == 'bzr':
        return vcs.vcs_bzr(remote, local)
    if vcstype == 'srclib':
