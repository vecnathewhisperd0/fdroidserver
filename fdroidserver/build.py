        ...
        try:
            ...
            raise BuildException(
                "Build failed for {0}:{1} - missing output files".format(
                    app.id, build.versionCode), str(output, 'utf-8')) from exc
        ftp.close()
...
def build_local(app, build, vcs, build_dir, output_dir, log_dir, srclib_dir, extlib_dir, tmp_dir, force, onserver, refresh):
    ...
    if common.get_file_extension(src) == 'apk':
        ...
        if vercode != build.versionCode:
            raise BuildException(("Unexpected version code in output;"
                                  " APK: '%d', "
                                  " Expected: '%d'")
                                 % (vercode, build.versionCode))

        logging.info("Successfully built version {version}/{vercode} of {appid} from {commit_id}"
                .format(version=version, vercode=vercode, appid=app.id, commit_id=commit_id))
...
def trybuild(app, build, build_dir, output_dir, log_dir, also_check_dir,
    ...
    logging.info("Building version %s of %s" % (build.versionCode, app.id))
